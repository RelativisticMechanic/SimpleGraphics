...
def Run(w, h, window_w, window_h, init, update, draw, onkeypress, onkeypressed, onkeyrelease, onmousemove, onexit):
    global GlobalScreen, GlobalWidth, GlobalHeight, GlobalClock, GlobalFont, GlobalWindow
    pygame.init()
    GlobalFont = pygame.font.SysFont('Arial', 16)
...
def PutPixel(x, y, r, g, b):
    GlobalScreen.set_at((x, y), pygame.Color((r,g,b)))

def DrawCircle(x, y, radius, r, g, b, filled = False, linewidth = 1):
    if(filled):
        pygame.draw.circle(GlobalScreen, pygame.Color(r,g,b), (x,y), radius, 0)
    else:
        pygame.draw.circle(GlobalScreen, pygame.Color(r,g,b), (x,y), radius, linewidth)

def DrawTriangle(x1, y1, x2, y2, x3, y3, r, g, b, filled = False, linewidth = 1):
    if(filled):
        pygame.draw.polygon(GlobalScreen, pygame.Color(r,g,b), [(x1,y1), (x2, y2), (x3, y3)], 0)
    else:
        pygame.draw.polygon(GlobalScreen, pygame.Color(r,g,b), [(x1,y1), (x2, y2), (x3, y3)], linewidth)

def DrawBlock(x, y, w, h, r, g, b, filled = False, linewidth = 1):
    if(filled):
        pygame.draw.rect(GlobalScreen, pygame.Color(r,g,b), pygame.Rect(x, y, w, h), 0)
    else:
        pygame.draw.rect(GlobalScreen, pygame.Color(r,g,b), pygame.Rect(x, y, w, h), linewidth)
...
def MakeTransparentImage(img, r,g,b):
    new_img = DuplicateImage(img)
    new_img.image_data.set_colorkey(pygame.Color(r,g,b))
...
def DrawString(str, x, y, r,g,b):
    textsurface = GlobalFont.render(str, False, (r,g,b))
    GlobalScreen.blit(textsurface, (x,y))

def DrawLine(x1, y1, x2, y2, r, g, b, width=1):
    pygame.draw.line(GlobalScreen, pygame.Color(r,g,b), (x1, y1), (x2, y2), width)
